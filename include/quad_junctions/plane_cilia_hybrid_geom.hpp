...
template <class Real> std::vector<CiliumCurveFlat<Real>> generate_cilia_carpet(
  ...
  const Real max_amp = (Real)0.4 * (2 * S);     // 0.4 * collar full side (= cell pitch 2S) = 0.8*S
  const Real edge_amp = (Real)0.2 * (L / (Real)Npatch);  // 0.2 * whole panel (cell pitch 2S); the small wiggle
                                                          // kept on box-edge cilia (also reserved as tilt-fit margin)
  const Real H_floor = (Real)0.15;              // last-resort shortening floor (tip stays >= this far from its wall)
  ...
  // 2. OUTWARD-TILTING CILIA -> DEAD STRAIGHT. Test-case variant: an edge cilium whose tilt carries it toward
  //    the nearby periodic box face (bottom +x, high-x columns / top -x, low-x columns) is set to ZERO tilt AND
  //    no wiggle, so it stands vertical at its cell center -- keeping all geometry well clear of x/y = 0, L.
  //    Interior / inward-tilting cilia are untouched (they still tilt + wiggle exactly as before).
  // --- OLD (tilt reduction: bisect tilt down only until the nominal tip just fits [buffer, L-buffer]) ---
  // Long n_tilt_cut = 0;
  // for (Long i = 0; i < N; i++) {
  //   const auto& c = C[i];
  ...
  Long n_tilt_cut = 0;                           // now counts cilia straightened (reused in the report line)
  for (Long i = 0; i < N; i++) {
    const auto& c = C[i];
    auto fits = [&](Real t) { const Real x = tip_world_x<Real>(c.cx, c.cy, R_shaft, Hreach[i], r_fil, Naz, t, c.dir_x, n_straight, n_trans, c.z_base, c.reflect);
      return (x - R_shaft) >= box_buffer && (x + R_shaft) <= (L - box_buffer); };
    if (fits(tilt[i])) continue;                 // interior / inward-tilting: unchanged
    tilt[i] = 0; straight[i] = 1; rebuild(i); n_tilt_cut++;   // outward-tilting: dead straight (tilt=0, no wiggle)
  }
  ...
  for (Long i = 0; i < N; i++) if (!straight[i]) draw(i);   // straightened outward cilia keep amp=0 (no wiggle)
  ...
  auto min_clear = [&](Long i) { Real best = (Real)1e30;
    ...
    return best; };
  // Whole tube+cap (centerline +- R_shaft, in BOTH x and y) must stay within [box_buffer, L-box_buffer].
  ...
  // EDGE cilia keep their (tilt-fit) lighter tilt and still wiggle, just at the smaller edge_amp (defined
  // above and reserved as tilt-fit margin, so a box-fitting orientation exists). The wiggle excurses +-amp
  // in BOTH directions (full sine period), so the reserved margin is what lets any orientation fit.
  Long n_regen = 0, n_edge = 0, n_short = 0, n_ampcut = 0;
  for (Long i = 0; i < N; i++) {
    if (straight[i] || !bad(i)) continue;   // never re-wiggle a straightened outward cilium
    ...
    if (exceeds_box(i)) {
      // keep the current (tilt-fit) tilt; small edge_amp wiggle oriented WALL-PARALLEL (theta=pi/2 => pure-y,
      // wig_p2=(0,1,0), adds nothing in the tilt/x direction) so it fits the box at the current tilt.
      amp[i] = edge_amp; theta[i] = const_pi<Real>() / 2; rebuild(i); samples_of(i, smp[i]);
      n_edge++;
    }
    // Phase C: resolve any remaining collision by shortening (keeps wiggle + tilt); interior collision-only
    //   cilia keep their FULL amplitude and just shorten. Shrink amp only if the box STILL can't be met (rare).
    bool ampcut = false, shortcut = false;
    for (int guard = 0; bad(i) && guard < 120; guard++) {
      if (exceeds_box(i) && amp[i] > (Real)1e-4) { amp[i] = std::max((Real)0, amp[i] - (Real)0.1*edge_amp); ampcut = true; }
      else { if (Hreach[i] <= H_floor + (Real)1e-9) break; Hreach[i] = std::max(H_floor, Hreach[i] - (Real)0.05); shortcut = true; }
  ...
  if (!comm.Rank())
    std::cout << "  generate_cilia_carpet: " << N << " cilia  seed=" << seed << "  amp in [" << amp_min
              << ", " << max_amp << ") (=[0.5,1]*0.4*2S, S=" << S << ")\n"
              << "    straightened (outward->vertical): " << n_tilt_cut << "   wiggle regenerations: " << n_regen
              << "   edge cilia (small amp " << edge_amp << ", kept tilt): " << n_edge
              << "   fallback amp-cut/shortened: " << n_ampcut << "/" << n_short << "\n"
