/**
 * Scheme comparison on a twisted cubed sphere: RP / Adaptive / Hybrid / Duffy.
 *
 * A copy of bench-cubed-sphere.cpp specialised to compare the four singular-quadrature SCHEMES on
 * one machine, rather than one scheme across three machines. The {Rome, Genoa, Icelake} columns of
 * that bench's Table 2 become {RP, Adaptive, Hybrid, Duffy} here: both the Green's-identity error
 * and the single-layer setup throughput are reported per scheme.
 *
 ...
 *
 * error-density (the off-node density interpolation error) from bench-cubed-sphere is intentionally
 * DROPPED here -- it is a resolution indicator, not a bound on the identity, and it is scheme
 * independent, so it adds nothing to a scheme comparison (and its serial computation is skipped).
 *
 ...
 * parser (scripts/parse-scheme-compare.py). The same format serves both modes; in conv mode `thr`
...
// Uref = u at the targets. Lifted verbatim from test-greens-conv / bench-cubed-sphere.
...
// counts. Emits @@OMP lines for the parser. Mirrors bench-cubed-sphere's scaling section.
